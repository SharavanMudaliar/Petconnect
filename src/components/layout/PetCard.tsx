...
export default function PetCard(
  ...
  return (
    <div className="lg:px-4 px-1  w-full md:w-1/2 lg:w-1/3 my-4 fadeInUp">
      <Card
        className="bg-base-100 shadow-xl text-yellow-500 bg-blue-400"
        bordered
      >
        <div className="p-6 flex flex-row justify-between bg-blue-400 text-yellow-500">
          {CardText(name)}
          <Link to={`/animal/${id}`}>
            <Button className="bg-blue-500 text-yellow-500" size="sm">
              More Info
            </Button>
          </Link>
        </div>
        <Card.Image
          ...
          className="object-cover h-80 w-full bg-blue-400 text-yellow-500"
        />
        <Card.Body className="flex flex-row justify-center bg-blue-400 text-yellow-500 ">
...
const CardText = (title: string) => (
  <span className="font-amatic text-3xl text-neutral font-bold text-ellipsis overflow-hidden whitespace-nowrap bg-blue-400 text-yellow-500">
    {nameCleaner(title)}
  </span>
);
